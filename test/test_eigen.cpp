...
#include "src/plot_trajectory/DrawTrajectory.h"
...
TEST_F(EigenLibTutorial, LinearAlgebra) {
    ...
    std::cout << "time of normal inverse is "
              << 1000. * static_cast<double>(clock() - time_stt) /
...
TEST_F(EigenLibTutorial, Geometry) {
    // Eigen/geometry module provides a variety of rotation and translation
    // representations 3d rotation matrix directly using Matrix3d or Matrix 3f
    Eigen::Matrix3d rotation_matrix = Eigen::Matrix3d::Identity();

    // The rotation vector uses AngleAxis, the underlying layer is not directly
    // Matrix, but the operation can be treated as a matrix (because the
    // operator is overloaded)
    Eigen::AngleAxisd rotation_vector(
        M_PI / 4.,
        Eigen::Vector3d(0, 0, 1)); // rotate 45 degrees along the Z-axis
    ...
    // Euler angle: You can convert the rotation matrix directly into Euler
    // angles
    Eigen::Vector3d euler_angles = rotation_matrix.eulerAngles(
        2, 1, 0); // ZYX order, ie roll pitch yaw order
    std::cout << "yaw pitch roll = " << euler_angles.transpose() << std::endl;

    // Euclidean transformation matrix using Eigen::Isometry
    Eigen::Isometry3d T =
        Eigen::Isometry3d::Identity(); // Although called 3d, it is essentially
                                       // a 4*4 matrix
    T.rotate(rotation_vector);         // Rotate accoring to rotation_vector)

    T.pretranslate(
        Eigen::Vector3d(1, 3, 4)); // Set the translation vector to (1, 3, 4)
    ...
    std::cout << "should be equal to "
              << (q * Eigen::Quaterniond(0, 1, 0, 0) * q.inverse())
                     .coeffs()
                     .transpose()
              << std::endl;
}
